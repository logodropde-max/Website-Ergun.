...
**Emre (Ebene 5) seit 23.09.:** in die Szene gerechnet von Higgsfield (`4k/szene-94d2b705.png`, Körperbau nach Emres Fotos, auf 93 % verkleinert = 1,77 m), **Kopf echt 1:1 aus dem Studiofoto** per `4k/kopf_echt.py` (bis knapp unters Kinn → `kopf-echt.png`), Hals von Higgsfield an seinen Kopf angepasst (`4k/szene-hals.png` = Job 94d2b705 + Halsbereich aus Job 617c5271), zusammengesetzt von `einsetzen.py`; `echtes_gesicht.py` = verworfene Mischvariante, eingesetzt mit `bilder/hero/4k/einsetzen.py` (Freistellen, Boden unter den Schuhen, alle Größen). `licht.py` gilt nur noch für die alte Figur. Montagespuren an Kamm und Vordergrund: `4k/reparatur.py`.
