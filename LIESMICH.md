...
**Emre (Ebene 5) seit 23.09.:** in die Szene gerechnet von Higgsfield (`4k/szene-94d2b705.png`, Körperbau nach Emres Fotos, auf 93 % verkleinert = 1,77 m), **Kopf 1:1 aus dem Studiofoto** (`4k/kopf-echt.png` aus `kopf_echt.py`, bis knapp unters Kinn) auf dem **Original-Hals der Figur** (`szene-94d2b705.png`), zusammengesetzt von `einsetzen.py`. Verworfen: `gesicht_nur.py` (veränderte das Gesicht), `szene-hals.png` (KI-Hals), `echtes_gesicht.py` (Mischung), eingesetzt mit `bilder/hero/4k/einsetzen.py` (Freistellen, Boden unter den Schuhen, alle Größen). `licht.py` gilt nur noch für die alte Figur. Montagespuren an Kamm und Vordergrund: `4k/reparatur.py`.
