...
- **24.09. 21:10 (Commit 5a58aa3): Fragebogen als Tabelle.** Formular `#anfrage.tabelle` (JS setzt `.ist-tabelle`), jede Frage `.zeile[data-schritt]` mit `.zeile__frage` (`.zeile__nr`, `p#frage-0X`, `small`) und `.zeile__antworten`; Optionen `.zellen` > `label.wahl` (echtes Radio/Checkbox unsichtbar darüber, `span` = Zelle, `::after` = Punkt/Häkchen). Farben: `--zelle: #2A2A27`, Felder `#1A1A17`. Kein Stepper mehr (Fortschritt, Zurück/Weiter entfernt), `[data-zeige-bei]` nur mit `.ist-tabelle` versteckt, Zusammenfassung `.mitnahme` aktualisiert sich live. Letzte Zeile `.zeile--kontakt` (Enter sendet nur dort ab). CSS-Block „Fragebogen als Tabelle (24.09. nachts)“ vor dem Auftritt-Skript; alte `.schritt`/`.bogen`/`.chip`-Regeln ohne Wirkung. Handy ≤ 560 px: eine Spalte.

## Datei-Upload im Formular (Stand 24.09., Claude Code)
WhatsApp-Links (`wa.me`) können nur Text tragen. Deshalb lädt der Fragebogen (Frage 05, Dateifeld `dateien`) gewählte Dateien sofort hoch und schreibt die Links in die Nachricht.
