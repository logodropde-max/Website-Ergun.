...
**Emre (Ebene 5) seit 23.09.:** in die Szene gerechnet von Higgsfield (`4k/szene-94d2b705.png`, Körperbau nach Emres Fotos, auf 93 % verkleinert = 1,77 m), **nur das Gesicht** 1:1 aus dem Studiofoto per `4k/gesicht_nur.py` (→ `szene-gesicht.png`, Hals/Haar/Ohren bleiben von der Figur), zusammengesetzt von `einsetzen.py`. Reserve: `kopf_echt.py` (ganzer Kopf), `szene-hals.png` (Halskorrektur), `echtes_gesicht.py` (Mischung, verworfen), eingesetzt mit `bilder/hero/4k/einsetzen.py` (Freistellen, Boden unter den Schuhen, alle Größen). `licht.py` gilt nur noch für die alte Figur. Montagespuren an Kamm und Vordergrund: `4k/reparatur.py`.
