...
(function () {
  ...
  function lichtX(licht, x) {
    if (licht === 'nacht') { var mx = m.W * 0.76; return Math.max(-1, Math.min(1, (mx - x) / (m.W * 0.25))); }
    return Math.max(-1, Math.min(1, (m.W * 0.5 - x) / (m.W * 0.22)));
  }
  /* Nähe zur Lichtquelle (für Streiflicht und Kanten im Gegenlicht) */
  function naehe(licht, x) { var lx = licht === 'nacht' ? m.W * 0.76 : m.W * 0.5; return Math.exp(-Math.pow((x - lx) / (m.W * (licht === 'tag' ? 0.6 : 0.32)), 2)); }
  ...
  function wolken(ebene) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (Math.abs(x - m.W * 0.76) < m.W * 0.08 && y < hoehe * 0.3) continue;
  ...
  function aufbauen() {
    ...
    m.mondX = m.W * (m.hoch ? 0.78 : 0.76);  /* Saturn rechts oben (Emre, 26.09.) */
